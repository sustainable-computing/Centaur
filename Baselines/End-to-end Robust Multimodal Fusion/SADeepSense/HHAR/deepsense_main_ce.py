...
def parse_option():
    ...
    parser.add_argument('--testmode', type=bool, default=True,
                        help='test mode only')
...
def set_loader(opt):
    ...
    #     num_workers=opt.num_workers, pin_memory=True, shuffle=True)
    train_loader, val_loader = data.getData()
   # print(train_loader)
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (input_data, labels) in enumerate(train_loader):
        ...
        if torch.cuda.is_available():
            ...
            #     print(param.data)
            #model = model.to(torch.FloatTensor)
        ...
        if (idx + 1) % opt.print_freq == 0:
            print('Train: [{0}][{1}/{2}]\t'
                  ...
                  'Acc@1 {top1.val:.3f} ({top1.avg:.3f})\t'.format(
                   epoch, idx + 1, len(train_loader)-1000, batch_time=batch_time,
...
def validate(val_loader, model, criterion, opt):
    ...
    with torch.no_grad():
        ...
        for idx, (input_data, labels) in enumerate(val_loader):
            # print(labels)
...
def main():
    ...
    #torch.seed(0) ## uncomment for reproducibility.
    opt = parse_option()
    # print('CALLED')
    ...
    if opt.testmode:
        ...
        train_loader, val_loader = set_loader(opt)
        model, criterion = set_model(opt)
    finalTest(save_file, validate, val_loader, criterion, opt)
